...
async function initProductsPage() {
  ...
  grid.innerHTML = Array.from({ length: 6 }, () => '<div class="card h-80 animate-pulse bg-slate-200/60"></div>').join('');
  ...
  try {
    ...
    grid.innerHTML = products.length
      ...
      : `<p class="col-span-full rounded-xl bg-white p-10 text-center text-slate-500">
           No parts matched. <a href="/products.html" class="link-all">Browse everything</a>
         </p>`;
  } catch (error) {
    grid.innerHTML = `<p class="col-span-full rounded-xl border border-amber-200 bg-amber-50 p-6 text-center text-sm font-semibold text-amber-900">${esc(error.message)}</p>`;
  }
}
    ...
    <article class="group card flex flex-col overflow-hidden transition hover:-translate-y-0.5 hover:shadow-lg">
      <a href="/product.html?slug=${encodeURIComponent(product.slug)}">
        <div class="ph ph-${(index % 4) + 1} h-52">
          ${discounted ? `<span class="absolute left-3 top-3 z-10 rounded-md bg-red-600 px-2 py-1 text-xs font-bold text-white">-${product.discountPercent}%</span>` : ''}
      ...
      <div class="flex flex-1 flex-col p-4">
        <p class="text-xs ${soldOut ? 'font-semibold text-red-600' : 'text-slate-500'}">${soldOut ? 'Out of stock' : `${product.stock} in stock`}</p>
        <h3 class="mt-1 min-h-[3.75rem] text-sm font-bold leading-snug text-slate-900">
          <a href="/product.html?slug=${encodeURIComponent(product.slug)}" class="line-clamp-3 hover:text-blue-600">${esc(product.title)}</a>
        </h3>
        ...
        <div class="mt-3 flex items-baseline gap-2">
          ...
          ${discounted ? `<span class="text-sm text-slate-400 line-through">${money(product.oldPriceCents)}</span>` : ''}
        </div>
        <button type="button" data-add-to-cart="${product.id}" ${soldOut ? 'disabled' : ''}
                class="btn mt-4 w-full ${soldOut ? 'cursor-not-allowed bg-slate-100 text-slate-400' : 'bg-blue-600 text-white hover:bg-blue-700'}">
...
async function initProductPage() {
  ...
  const fallback = (title, body) => `
    <div class="card p-10 text-center">
      <h1 class="text-xl font-extrabold text-slate-900">${esc(title)}</h1>
      <p class="mt-2 text-sm text-slate-500">${esc(body)}</p>
  ...
  try {
    ...
    mount.innerHTML = `
      <nav class="mb-6 text-sm text-slate-500" aria-label="Breadcrumb">
        <a href="/products.html" class="hover:text-blue-600">Parts</a>
        ${product.categorySlug ? ` / <a href="/products.html?category=${encodeURIComponent(product.categorySlug)}" class="hover:text-blue-600">${esc(product.category)}</a>` : ''}
      </nav>

      <div class="grid gap-8 lg:grid-cols-2">
        <div class="ph ph-2 aspect-[4/3] rounded-2xl">
          ${discounted ? `<span class="absolute left-4 top-4 z-10 rounded-md bg-red-600 px-2.5 py-1 text-sm font-bold text-white">-${product.discountPercent}%</span>` : ''}
          <span class="ph-icon">${PART_ICON}</span>
          ${imageTag(product.imagePath, { alt: esc(product.title), className: 'absolute inset-0 h-full w-full rounded-2xl object-cover', lazy: false })}
        </div>

        <div>
          <p class="eyebrow">${esc(product.brand)} &bull; ${esc(product.category ?? 'Parts')}</p>
          <h1 class="mt-2 text-2xl font-extrabold leading-tight tracking-tight text-slate-900 sm:text-3xl">${esc(product.title)}</h1>

          ${product.partNumber ? `<p class="mt-3 text-sm text-slate-500">Part number <span class="font-mono font-semibold text-slate-700">${esc(product.partNumber)}</span></p>` : ''}

          <div class="mt-5 flex items-baseline gap-3">
            <span class="text-3xl font-extrabold text-slate-900">${money(product.priceCents)}</span>
            ${discounted ? `<span class="text-lg text-slate-400 line-through">${money(product.oldPriceCents)}</span>` : ''}
          </div>

          <p class="mt-2 text-sm ${soldOut ? 'font-semibold text-red-600' : 'text-green-700'}">
            ${soldOut ? 'Out of stock' : `${product.stock} in stock — ready to ship`}
          </p>

          ${product.description ? `<p class="mt-5 leading-relaxed text-slate-600">${esc(product.description)}</p>` : ''}

          <div class="mt-7 flex flex-col gap-3 sm:flex-row">
            <button type="button" data-add-to-cart="${product.id}" ${soldOut ? 'disabled' : ''}
                    class="btn flex-1 ${soldOut ? 'cursor-not-allowed bg-slate-100 text-slate-400' : 'bg-blue-600 text-white hover:bg-blue-700'}">
              ${soldOut ? 'Out of stock' : 'Add to cart'}
            </button>
            <a href="/contact.html" class="btn-outline flex-1">Ask about fitment</a>
          </div>

          <div class="mt-6 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm leading-relaxed text-slate-600">
            <strong class="text-slate-900">Not sure it fits?</strong>
...
async function renderCart() {
  ...
  if (lines.length === 0) {
    mount.innerHTML = `
      <div class="card p-12 text-center">
        <p class="text-lg font-bold text-slate-900">Your cart is empty</p>
        <p class="mt-2 text-slate-500">Find the part you need and it will show up here.</p>
  ...
  try {
    ...
    mount.innerHTML = `<div class="rounded-2xl border border-red-200 bg-red-50 p-6 text-sm font-semibold text-red-800">${esc(error.message)}</div>`;
  }
}

const cartRow = (item, index) => `
  <div class="flex gap-4 p-4">
    <div class="ph ph-${(index % 4) + 1} h-24 w-24 shrink-0 rounded-xl">
      ...
      ${imageTag(item.imagePath, { alt: esc(item.title), className: 'absolute inset-0 h-full w-full rounded-xl object-cover' })}
    </div>

    <div class="min-w-0 flex-1">
      <h3 class="line-clamp-2 text-sm font-bold text-slate-900">${esc(item.title)}</h3>
      <p class="mt-1 text-xs text-slate-500">${esc(item.brand ?? '')}</p>
      <p class="mt-1 text-sm text-slate-600">${money(item.unitPriceCents)} each</p>
      ...
      <div class="mt-3 flex items-center gap-3">
        <div class="inline-flex items-center rounded-lg border border-slate-300">
          <button type="button" data-qty="${item.productId}" data-delta="-1" aria-label="Decrease quantity"
                  class="grid h-8 w-8 place-items-center text-slate-600 hover:bg-slate-50">−</button>
        ...
        <button type="button" data-remove="${item.productId}" class="text-xs font-semibold text-slate-500 hover:text-red-600">
          Remove
        </button>
      </div>
    </div>

    <p class="shrink-0 text-base font-extrabold text-slate-900">${money(item.lineTotalCents)}</p>
  </div>`;
...
const cartSummary = (quote) => `
  <div class="card sticky top-24 p-6">
    ...
    <dl class="mt-5 space-y-3 text-sm">
      <div class="flex justify-between"><dt class="text-slate-600">Subtotal</dt><dd class="font-semibold text-slate-900">${money(quote.subtotalCents)}</dd></div>
      <div class="flex justify-between">
        <dt class="text-slate-600">Shipping</dt>
        <dd class="font-semibold ${quote.shippingCents === 0 ? 'text-green-700' : 'text-slate-900'}">
          ${quote.shippingCents === 0 ? 'Free' : money(quote.shippingCents)}
        </dd>
      </div>
      ${quote.taxCents ? `<div class="flex justify-between"><dt class="text-slate-600">Tax</dt><dd class="font-semibold text-slate-900">${money(quote.taxCents)}</dd></div>` : ''}
    </dl>

    <div class="mt-4 flex items-baseline justify-between border-t-2 border-slate-900 pt-4">
      <span class="text-base font-extrabold text-slate-900">Total</span>
      <span class="text-2xl font-extrabold text-slate-900">${money(quote.totalCents)}</span>
    </div>
  ...
  try {
      ...
      <ul class="space-y-3">
        ${quote.items
          ...
          <li class="flex justify-between gap-4 text-sm">
            <span class="text-slate-600">${esc(item.title)} <span class="text-slate-400">× ${item.quantity}</span></span>
            <span class="shrink-0 font-semibold text-slate-900">${money(item.lineTotalCents)}</span>
          </li>`,
          )
          .join('')}
      </ul>
      <dl class="mt-5 space-y-2 border-t border-slate-200 pt-4 text-sm">
        <div class="flex justify-between"><dt class="text-slate-600">Subtotal</dt><dd class="font-semibold">${money(quote.subtotalCents)}</dd></div>
        <div class="flex justify-between"><dt class="text-slate-600">Shipping</dt><dd class="font-semibold">${quote.shippingCents === 0 ? 'Free' : money(quote.shippingCents)}</dd></div>
        ${quote.taxCents ? `<div class="flex justify-between"><dt class="text-slate-600">Tax</dt><dd class="font-semibold">${money(quote.taxCents)}</dd></div>` : ''}
      </dl>
      <div class="mt-4 flex items-baseline justify-between border-t-2 border-slate-900 pt-4">
    ...
    methods.innerHTML = quote.paymentMethods.length
      ? quote.paymentMethods
          .map(
            (method, index) => `
        <label class="flex cursor-pointer items-center gap-3 rounded-xl border border-slate-300 p-4 transition has-[:checked]:border-blue-600 has-[:checked]:bg-blue-50">
          <input type="radio" name="paymentMethod" value="${esc(method.id)}" ${index === 0 ? 'checked' : ''}
                 class="h-4 w-4 text-blue-600 focus:ring-blue-500">
          <span class="text-sm font-semibold text-slate-900">${esc(method.label)}</span>
        </label>`,
          )
          .join('')
...
async function initMyOrders() {
  ...
  mount.innerHTML = '<div class="card h-40 animate-pulse bg-slate-200/60"></div>';
      ...
      ? orders
        ...
        <a href="/order.html?id=${order.id}&token=${encodeURIComponent(order.accessToken)}"
          ...
          <div class="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p class="font-mono text-sm font-bold text-slate-900">${esc(order.orderNumber)}</p>
              <p class="mt-1 text-xs text-slate-500">${formatDate(order.createdAt)} &bull; ${order.items.length} item${order.items.length === 1 ? '' : 's'}</p>
            </div>
            <div class="flex items-center gap-3">
              <span class="text-lg font-extrabold text-slate-900">${money(order.totalCents, order.currency)}</span>
          ...
          .join('')
      : `<div class="card p-12 text-center">
           <p class="text-lg font-bold text-slate-900">No orders yet</p>
           <p class="mt-2 text-slate-500">When you buy a part, it will show up here.</p>
           <a href="/products.html" class="btn-primary mt-6">Browse parts</a>
         </div>`;
  } catch (error) {
      ...
      <div class="card p-6 sm:p-8">
        <div class="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p class="mt-2 text-sm text-slate-500">Placed ${formatDate(order.createdAt)}</p>
          </div>
          ${statusBadge(order.status)}
        </div>
        ...
        ${
          order.trackingNumber
            ? `<div class="mt-6 rounded-xl border border-green-200 bg-green-50 p-4">
                 <p class="text-xs font-bold uppercase tracking-wide text-green-800">Tracking</p>
                 <p class="mt-1 font-mono text-sm font-semibold text-green-900">${esc(order.trackingNumber)}</p>
                 ${order.carrier ? `<p class="mt-1 text-xs text-green-700">via ${esc(order.carrier)}</p>` : ''}
               </div>`
            : ''
        }
        ...
        <div class="mt-6 divide-y divide-slate-200 border-y border-slate-200">
          ${order.items
            ...
            <div class="flex justify-between gap-4 py-4">
              <div class="min-w-0">
                <p class="text-sm font-bold text-slate-900">${esc(item.title)}</p>
                <p class="mt-0.5 text-xs text-slate-500">
                  ${esc(item.brand ?? '')}${item.partNumber ? ` &bull; ${esc(item.partNumber)}` : ''} &bull; Qty ${item.quantity}
                </p>
              </div>
              <p class="shrink-0 text-sm font-bold text-slate-900">${money(item.lineTotalCents, order.currency)}</p>
            </div>`,
            )
            .join('')}
        </div>
        ...
        <dl class="mt-5 space-y-2 text-sm">
          <div class="flex justify-between"><dt class="text-slate-600">Subtotal</dt><dd class="font-semibold">${money(order.subtotalCents, order.currency)}</dd></div>
          <div class="flex justify-between"><dt class="text-slate-600">Shipping</dt><dd class="font-semibold">${order.shippingCents ? money(order.shippingCents, order.currency) : 'Free'}</dd></div>
          ${order.taxCents ? `<div class="flex justify-between"><dt class="text-slate-600">Tax</dt><dd class="font-semibold">${money(order.taxCents, order.currency)}</dd></div>` : ''}
          ${order.refundedCents ? `<div class="flex justify-between text-green-700"><dt>Refunded</dt><dd class="font-semibold">−${money(order.refundedCents, order.currency)}</dd></div>` : ''}
          <div class="flex justify-between border-t-2 border-slate-900 pt-3 text-base">
            <dt class="font-extrabold text-slate-900">Total</dt>
            <dd class="font-extrabold text-slate-900">${money(order.totalCents, order.currency)}</dd>
          </div>
        </dl>

        <div class="mt-6 rounded-xl bg-slate-50 p-4 text-sm leading-relaxed text-slate-600">
          <p class="mb-1 text-xs font-bold uppercase tracking-wide text-slate-900">Shipping to</p>
...
const refundPanel = (order) => `
  <details class="card mt-6 p-6">
    <summary class="cursor-pointer text-sm font-bold text-slate-900">Something wrong? Request a refund</summary>
